...
public class SprayPaintItemNetExt: NetworkBehaviour {
    const int UtilityItemSlotIndex = 50;

    static readonly FieldInfo? utilitySlotField =
        typeof(PlayerControllerB).GetField("ItemOnlySlot", BindingFlags.Instance | BindingFlags.Public);

    static GrabbableObject? UtilitySlotItem(PlayerControllerB p) =>
        utilitySlotField?.GetValue(p) as GrabbableObject;

    SprayPaintItem instance => GetComponent<SprayPaintItem>();
    public bool HeldByLocalPlayer {
        get {
            var p = instance.playerHeldBy;
            if (p == null || !p.IsLocalPlayer()) return false;
            if (UtilitySlotItem(p) == instance) return true;
            return p.ItemSlots != null && p.ItemSlots.Any(item => item == instance);
        }
    }
    public bool InActiveSlot {
        get {
            ...
            if (p.currentItemSlot == UtilityItemSlotIndex && utilitySlotField != null) {
                return UtilitySlotItem(p) == instance;
            }
            var slots = p.ItemSlots;
            if (slots == null || p.currentItemSlot < 0 || p.currentItemSlot >= slots.Length) {
                return false;
            }
            return slots[p.currentItemSlot] == instance;
        }
    }
    ...
    public override void OnNetworkSpawn() {
        ...
        ColorPalette = new List<Color>(Plugin.MineColors);
        if (IsServer) {
            var rand = new System.Random();
            CurrentColor = ColorPalette[rand.Next(ColorPalette.Count)];
        }
    ...
    public void UpdateParticles() {
        ...
        particleMain.startSizeMultiplier = 0.5f * particleSize;
        particleShape.scale = new Vector3(particleSize, particleSize, particleSize);
    ...
    public void UpdateTooltips() {
        ...
        int i = lines.Length > 3 ? 3 : lines.Length - 1;
        lines[i].text = $"Size : {PaintSize:0.0}";
    }
