...
public partial class Plugin {
    ...
    public static bool ExtendedMineTest { get; private set; }

    static readonly Color[] ExtendedPaletteColors = {
        new Color32(0xFF, 0x00, 0x00, 0xFF), //빨강(바닐라)
        new Color32(0xFF, 0x69, 0xB4, 0xFF), //핫핑쿠
        new Color32(0xFF, 0xEB, 0x3B, 0xFF), //노랑(바닐라)
        new Color32(0x00, 0xC8, 0x53, 0xFF), //초록(바닐라)
        new Color32(0x00, 0xBC, 0xD4, 0xFF), //아쿠아
        new Color32(0x7B, 0x1F, 0xA2, 0xFF), //보라(바닐라
        new Color32(0xFF, 0xFF, 0xFF, 0xFF), //흰색
    };

    static readonly Color[] SimplePaletteColors = {
        new Color32(0xFF, 0x00, 0x00, 0xFF),
        new Color32(0xFF, 0xEB, 0x3B, 0xFF),
        new Color32(0x00, 0xC8, 0x53, 0xFF),
        new Color32(0x7B, 0x1F, 0xA2, 0xFF),
    };

    internal static IReadOnlyList<Color> MineColors =>
        SessionData.ExtendedMineTest ? ExtendedPaletteColors : SimplePaletteColors;
...
""");
        ...
        ConfEntry("General", nameof(ExtendedMineTest), true, "When true, color palette is red, pink, yellow, green, aqua, purple, white. When false, only red, yellow, green, purple.", bool.TryParse, hostControlled: true, configKey: "ExtendedColor");
    ...
    private void ConfEntry<T>(string category, string name, T defaultValue, string description, ParseConfigValue<T> tryParse, bool hostControlled = false, string? configKey = null) {
        ConfEntryInternal(category, name, defaultValue, description, tryParse, hostControlled, null, configKey);
    }
    private void ConfEntry<T>(string category, string name, T defaultValue, string description, ParseConfigValue<T> tryParse, T vanillaValue, bool hostControlled = false, string? configKey = null) {
        ConfEntryInternal(category, name, defaultValue, description, tryParse, hostControlled, ConfEntryToString(vanillaValue), configKey);
    }
    private void ConfEntryInternal<T>(string category, string propertyName, T defaultValue, string description, ParseConfigValue<T> tryParse, bool hostControlled = false, string? vanillaValueText = null, string? configKey = null) {
        var property = typeof(Plugin).GetProperty(propertyName);
        ...
        var config = Config.Bind<string>(category, configKey ?? propertyName, "default", desc);
